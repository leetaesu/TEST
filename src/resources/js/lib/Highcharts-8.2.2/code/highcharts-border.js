	...
	(function(H) {
		H.wrap(H.seriesTypes.column.prototype, 'translate', function(proceed) {
			...
			this.points.forEach(function(point) {
				if(options.borderRadiusTopLeft || options.borderRadiusTopRight || options.borderRadiusBottomRight || options.borderRadiusBottomLeft || options.customColumnRadius) {
					...
					if(options.customColumnRadius) {
						if (point.y > 0) {
							radiusTopLeft = H.relativeLength(options.customColumnRadius || 0, w);
							radiusTopRight = H.relativeLength(options.customColumnRadius || 0, w);
							radiusBottomRight = 0;
							radiusBottomLeft = 0;
						} else {
							radiusTopLeft = 0;
							radiusTopRight = 0;
							radiusBottomRight = H.relativeLength(options.customColumnRadius || 0, w);
							radiusBottomLeft = H.relativeLength(options.customColumnRadius || 0, w);
						}
					}
